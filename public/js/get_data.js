...
function playSound(instrument, sound) {
  const soundId = `${instrument.toLowerCase().replace(/ /g, "")}${sound.replace(
    / /g,
    ""
  )}`;
...
function fetchInstrumentData() {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      console.log("Datos recibidos desde el servidor:", instrumentos); // Mostrar datos en la consola
      instrumentosPrincipales.forEach((nombre) => {
        const instrumentoData = instrumentos.find(
          (instr) => instr.nombre_instrumento === nombre
        );
        const valor = instrumentoData ? instrumentoData.valor : 1;
        const sonido = instrumentoData ? instrumentoData.sonido : "";
        const instrumentoDiv = document.getElementById(
          nombre.replace(/ /g, "")
        );

        if (instrumentoDiv) {
          instrumentoDiv.className =
            "instrumento " + (valor == 2 ? "tocado" : "no-sonido");
          instrumentoDiv.textContent = `${nombre} - ${
            valor == 2 ? sonido : "No Emite Sonido"
          }`;
